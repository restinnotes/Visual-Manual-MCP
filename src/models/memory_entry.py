...
class MemoryEntry(BaseModel):
    ...
    section: Optional[str] = Field(
        None,
        description="Document section/chapter title for hierarchical clustering"
    )
    relations: List[dict] = Field(
        default_factory=list,
        description="Knowledge graph triplets: [{'subject': 'x', 'relation': 'y', 'object': 'z'}]"
    )

    # [RAPTOR Layer] - Hierarchical summary tree support
    raptor_level: int = Field(
        default=0,
        description="RAPTOR tree level: 0=atomic fact, 1=section summary, 2=root summary"
    )
    parent_ids: List[str] = Field(
        default_factory=list,
        description="Parent node IDs for tree traversal"
    )
